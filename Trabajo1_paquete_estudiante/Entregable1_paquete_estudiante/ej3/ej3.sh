#!/usr/bin/env bash
# ============================================================
# Trabajo 1 - Ej3: Top-3 ciudades con mas ventas
...
#
# Se corre parado DENTRO de la carpeta ej3/, junto a mapper.py y reducer.py.
# ============================================================
...
docker cp mapper.py                  namenode:/tmp/mapper_ej3.py
docker cp reducer.py                 namenode:/tmp/reducer_ej3.py

echo "==> 2. Subiendo el csv a HDFS..."
docker exec namenode hdfs dfs -mkdir -p /ej3/entrada
docker exec namenode hdfs dfs -put -f /tmp/transacciones.csv /ej3/entrada/
...
docker exec namenode hdfs dfs -rm -r -f /ej3/salida
...
# -numReduceTasks 1 es OBLIGATORIO aqui: el top-3 es un ranking GLOBAL, y solo
# se puede calcular bien si un unico reducer ve los totales de TODAS las
# ciudades. Con mas de un reducer, cada uno veria solo una parte de las
# ciudades y armaria su propio top-3 parcial (incorrecto).
docker exec namenode hadoop jar "$STREAMING_JAR" \
  -files /tmp/mapper_ej3.py,/tmp/reducer_ej3.py \
  -mapper "python3 mapper_ej3.py" \
  -reducer "python3 reducer_ej3.py" \
  -numReduceTasks 1 \
  -input /ej3/entrada/transacciones.csv \
  -output /ej3/salida

echo ""
echo "==> 5. Resultado (top-3 ciudades con mas ventas):"
docker exec namenode hdfs dfs -cat /ej3/salida/part-00000
